...
export function ProdukDetailClient({ produkId }: { produkId: string }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api<{ produk: ProdukDetail }>(`/api/produk/${produkId}`);
        setP(res.produk);
        const t = await api<{ items: ProdukListItem[] }>(
          `/api/produk?kategoriId=${res.produk.kategoriId}&limit=6`
        );
        setTerkait(t.items.filter((x) => x.id !== produkId).slice(0, 5));
      } catch (err) {
